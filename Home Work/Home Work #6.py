...
if __name__ == "__main__":
    ...
    while True:
        ...
        if desision == "add":
           ...
           remove_name = input("Please enter name you wish to delete: ")
           for dj in djs:
               if dj["name"] == remove_name:
                   djs.remove(dj)
                   print("Dj has been removed")
        elif desision == "update":
            update_name = input("Enter Dj's name you like to update info:")
            for dj in djs:
                if dj["name"] == update_name:
                    djs.remove(dj)
                    print(f'Enter DJ {update_name} info to update')
                    new_dj = add_dj(djs)
                    if new_dj:
                        print(f"DJ {new_dj['name']} is updated!")
        elif desision == "exit":
            print("Exiting...")
            break
        else:
